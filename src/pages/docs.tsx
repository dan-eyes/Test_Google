...
import { Search, Menu, Rocket, Package, ShieldCheck, Truck, CreditCard, Puzzle, ChevronRight, LayoutDashboard } from "lucide-react"
...
export function Docs() {
  ...
  return (
    <div className={cn("fixed inset-0 z-[100] font-sans flex transition-colors overflow-hidden", theme === 'dark' ? 'dark text-zinc-300 bg-[#0a0a0a]' : 'text-zinc-700 bg-[#f9fafb]')}>
      ...
      {/* STRUTTURA A COLONNE (Dashboard Shell) */}
      <div className="flex flex-1 overflow-hidden w-full relative h-screen">
                        ...
                        {section.items.map((item) => (
                          <li key={item.slug} className="w-full">
                            <Link
                              ...
                              className="block w-full px-4 py-1.5 text-[13px] transition-colors text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200/50 dark:hover:bg-white/[0.05] hover:text-zinc-900 dark:hover:text-white break-words leading-snug"
                            >
                              {item.title}
                            </Link>
                          </li>
        ...
        {/* COLONNA CENTRALE (DASHBOARD CARD) */}
        {/* ECCO IL FIX: aperto correttamente come <main> invece che come <div> */}
